...
async def on_message(message):
  ...
  if converted_message != message.content:
    sent_message = await message.channel.send(
        f'{message.author.mention} Converted link: {converted_message}')
...
def convert_links(content):
  ...
  instagram_replacement = r'https://www.ddinstagram.com/'
...
async def delete_unconverted_message(original_message, converted_message):
  try:
    ...
    author_message = await original_message.channel.fetch_message(
        original_message.id)
